...
let getGcd = () => {
  ...
  let a = Math.round(Math.random() * 420);
  let b = Math.round(Math.random() * 420);
  gcdButton.innerHTML = "GCD of "+a+", "+b+": "+gcd(a,b);
}

let getFib = () => {
  let fibButton = document.getElementById("fib");
  let a = Math.round(Math.random() * 420);
  fibButton.innerHTML = a+"th Fib number: "+fib(a);
}

let getFact = () => {
  let factButton = document.getElementById("fact");
  let a = Math.round(Math.random() * 420);
  factButton.innerHTML = "Factorial of "+a+": "+factorial(a);
}

document.getElementById("gcd").addEventListener('click', getGcd);
document.getElementById("fib").addEventListener('click', getFib);
document.getElementById("fact").addEventListener('click', getFact);
